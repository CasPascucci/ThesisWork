...
x0 = [1; 6.1; 5]; % Initial around E-Guidance
...
af_dim = [0; 0; 0];        % Final acceleration (m/s²)
...
X0 = [r; v; m0];
tspan = [0, tgo_opt];
tspan = linspace(0, tgo_opt, 1000);

% ODE solver options for high accuracy
odeoptions = odeset('RelTol', 1e-8, 'AbsTol', 1e-8);
[c1,c2] = computeCoefficients(r, v, tgo_opt, gamma1Opt, gamma2Opt, afStar, g, rfStar, vfStar);
% Simulate the trajectory
[t_traj, state_traj] = ode45(@(t, X) trajectoryEq51_fixed(t, X, gamma1Opt, gamma2Opt, ...
    tgo_opt, afStar, g, isp, c1, c2), ...
    tspan, X0, odeoptions);
...
initial_r = state_traj(1, 1:3).';
initial_v = state_traj(1, 4:6).';
[c1, c2] = computeCoefficients(initial_r, initial_v, tgo_opt, gamma1, gamma2, afStar, g, rfStar, vfStar);
...
% Figure 4, Throttle Profile
figure(4); hold on
plot(t_traj*T_ref, (vecnorm(aT_list.*mass_list,2,2)*A_ref*M_ref)/max_thrust_dim);
yline(max_thrust_dim/max_thrust_dim, 'r--', 'LineWidth', 1, 'DisplayName', 'Max Thrust');
yline(min_thrust_dim/max_thrust_dim, 'r--', 'LineWidth', 1, 'DisplayName', 'Min Thrust');
xlabel('Time');
...
% Figure 5: Mass depletion over time
figure(5);
...
function dXdt = trajectoryEq51_fixed(t, X, gamma1, gamma2, tgo0, af_star, g, isp, c1, c2)
    r    = X(1:3);
    V    = X(4:6);
    mass    = X(7);

% Current working file as of 7/23, adding in the changes from the 7/22
% meeting
clear; clc; close all;
...
x0 = [1; 6.1; 2]; % Initial around E-Guidance
%x0 = [1; 12.1; 2]; % Initial around Apollo Guidance
%x0 = [2; 8.1; 1]; % New initial guess folllowing gamma kr relation
...
vf_dim = [0; 0; 0];             % Target velocity (m/s)
...
T_ref = sqrt(L_ref/g_const);      % Reference time (s)
A_ref = g_const;          % Reference acceleration (m/s²)
V_ref = L_ref / T_ref;            % Reference velocity (m/s)
M_ref = 62000;            % Reference mass (kg)
...
[x_opt, fval] = fmincon(...
    @(params) objectiveEq51Closed(params, afStar, g, rfStar, r, vfStar, v, m0, isp), ...
...
[t_traj, state_traj] = ode45(@(t, X) trajectoryEq51(t, X, gammaOpt, krOpt, ...
    tgo_opt, afStar, g, rfStar, vfStar, isp), ...
    tspan, X0, odeoptions);
...
vf_real_norm_dim = sqrt(state_traj(end,4)^2 + state_traj(end,5)^2 + ...
                       state_traj(end,6)^2) * V_ref*sign(state_traj(end,6));
xf_real_dim = state_traj(end, 1) * L_ref;
yf_real_dim = state_traj(end, 2) * L_ref;
...
function cost = objectiveEq51Closed(params, af_star, g, rf_star, r, vf_star, v, m, isp)
    gamma = params(1);
    kr = params(2);
    tgo0 = params(3);  % Initial time-to-go
    ...
    g_nondim = g(3);  % Should be -1
    mass_vec = zeros(N,1);
    mass_vec(1) = m;  % Initial non-dimensional mass (m0)
    
    % Current state variables (like the simulation)
    current_r = r;
    current_v = v;
    ...
    for i = 1:N-1
        ...
        % Calculate current time-to-go (decreases as time progresses)
        tgo = max((tgo0 - tspan(i)), 0.001);
        
        % Recalculate coefficients at each step using CURRENT state (like simulation)
        [c1, c2] = computeCoefficients(current_r, current_v, tgo, gamma1, gamma2, af_star, g, rf_star, vf_star);
        aT = af_star + c1*tgo^gamma1 + c2*tgo^gamma2;
        
        % Mass flow calculation (same as simulation)
        F_mag = norm(aT) * mass_vec(i);
        dm_dt = -F_mag / (isp * (-g_nondim));
        mass_vec(i+1) = mass_vec(i) + dm_dt * dt;
        
        % Update state for next iteration (match simulation dynamics)
        current_v = current_v + aT * dt;  % Include gravity like simulation
        current_r = current_r + current_v * dt;
    end
    
    % Fuel consumed in non-dimensional units
    fuel_consumed_nondim = m - mass_vec(end);
    cost = fuel_consumed_nondim;
end
%----------------------------------------------------------------------------------------------
...
%----------------------------------------------------------------------------------------------
function dXdt = trajectoryEq51(t, X, gamma, kr, tgo0, af_star, g, rf_star, vf_star, isp)

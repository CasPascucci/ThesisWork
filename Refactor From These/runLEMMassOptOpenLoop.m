function S = runLEMMassOptOpenLoop(x0, cfg)
%RUNLEMMASSOPTOPENLOOP Optimize FP guidance for the LEM and simulate
%   S = runLEMMassOptOpenLoop(x0, cfg)
%
% Inputs
%   x0  [3x1] initial guess for [gamma; kr; tgo]
%   cfg struct with optional overrides, fields include:
...
% Tgo for 762.3 seconds is 9.70249498737309 if g=1.62

   % ======================= Bounds ====================
    lb = getfieldOr(cfg, 'lb', [1, 6, 3]); % Modified version of getfield that also allows a default option if N/A. At bottom of file
    ub = getfieldOr(cfg, 'ub', [1.0, 25, 20]);
    % fmincon options
    ...
    deltaTgoDim = getfieldOr(cfg, 'deltaTgoDim', 10); % I don't like how much results change with this, but maybe thats fair
    ...
    obj = @(params) objectiveFunction(params, afStar, rfStar, r0, vfStar, v0, dtgo, gConst);
...
%  ========================================================================
    ...
    S.targets = struct('rfVirt', rfVirt, 'vfVirt', vfVirt, 'afVirt', afVirt);
...
function cost = objectiveFunction(params, afStar, rfStar, r, vfStar, v, dtgo, g)
    gamma  = params(1);
    kr     = params(2);
    tgo0   = params(3);
    tgoVirt = tgo0 + dtgo;
    ...
    [c1, c2] = computeCoefficients(r, v, tgoVirt, gamma1, gamma2, afStar, rfStar, vfStar, g);
    ...
    %J1 = simpsonIntegral(tspan(1:idx), aT_norm(1:idx)); %Use this for
                                                %cost to be fuel optimizing
...
function [rfVirtual, vfVirtual, afVirtual, tgoVirtual] = computeBeyondTerminationTargeting(r, v, gamma, kr, rfStar, vfStar, afStar, delta_t, tgo_true, g)
    ...
    % coln = max(vecnorm(M,2,1), 1e-12);   % column 2-norms
    % Ms   = M ./ coln;                    % scale columns to ~1
    % ridge = 1e-12;                       % tiny Tikhonov
    % solution = (Ms.'*Ms + ridge*eye(9)) \ (Ms.'*b_vec);
    % solution = solution ./ coln(:);      % unscale
